...
Owns:
- OrderBook WebSocket subscription  →  FairValueEngine + GltSpreadEngine
- AggTrade WebSocket subscription   →  GltSpreadEngine intensity calibration
...
class MMService:
    ...
    Subscribes to the orderbook and trade streams for *exchange*/*symbol*,
    runs FairValueEngine and GltSpreadEngine against them, and dispatches
    QuoteState updates to registered listeners on every L2 tick.
    """
    ...
    def get_fair_price(self) -> FairPriceState | None:
        return self._fair_engine.state
    ...
    async def run(self) -> None:
        self.lg.info("mm_service_start")
        await asyncio.gather(self._book_tracker.run(), self._trade_tracker.run())
